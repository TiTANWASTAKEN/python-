...
@app.route('/screen1')
def screen1():
    base_folder_path = r"D:\version 2"
    current_date = datetime.now().strftime('%d-%m-%Y')  # Format it as needed
    folder1_path = os.path.join(base_folder_path, current_date,)
    ...
    current_date_display = datetime.now().strftime('%m-%d-%Y')  # Format it as needed

    return render_template('screen1.html', file_list=file_list, current_month=current_month, current_date=current_date_display)
